...
const Recommend = () => {
    ...
    //删除分类
    let todel = (users: any) => {
        request.post('/api/carousel/deletCarousel',{
            data:{
                id: users._id,
            }
        }).then(res =>{
        ...
        request.get('/api/carousel').then((res: any) =>{
            console.log(res)
    ...
    let onFinishs = (values: any) => {
      setShow1(false);
      if(form2 === null){
        request.post('/api/carousel/changeCarousel',{
            data:{
                id: form1,
                url:values.className
            }
        }).then(res =>{
            console.log(res)
        })
    ...
    let onFinishs1 = (values: any) => {
        ...
        if(values.classLastName){
            request.post('/api/carousel/addCarousel',{
                data:{
                    url: values.className
  ...
  let formatDate = (timeStamp: any) => {
    ...
    <div style={{ width: 50 }}>
      ...
      <Button value="large" size='large' onClick={showModal} type="primary">添加图片</Button>
    </div>
    <div>
      <Table dataSource={users.slice((current - 1)*pageSize, current*pageSize)} pagination={false}>
        <Column title="#" dataIndex="key" key="key" />
        <Column title="图片" dataIndex="url" key="className" 
            render={(url) => (
              // console.log(level),                                                                                                                      
             <img src={url} style={{width:300,height:120}}></img> 
